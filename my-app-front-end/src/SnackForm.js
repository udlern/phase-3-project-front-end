...
function SnackForm() {
  ...
  const displayEmployees = employeeList.map((employee) => {
    return (
  ...
  function handleSnackSubmit(event) {
    ...
    const reqBody = {
      ...
      body: JSON.stringify({ employee_id: employeeId, name: snackName, keto: keto, gluten_free: glutenFree, vegetarian: veggie, dairy_free: dairyFree, nut_free: nutFree, picture: snackImage}),
    };
    fetch(API, reqBody)
      .then((resp) => resp.json())
  ...
  return (
    <div className="snack-form">
      <form>
        {/* <h2 className="form-headers">New Snack Form</h2> */}
        <Button onClick={handleOpen}>Click to add a snack!</Button>
      <Modal
        open={open}
        onClose={handleClose}
      ...
      <InputLabel id="demo-simple-select-label">Employees</InputLabel>
        ...
        <label>
        <TextField id="snack-name-input" value={snackName} onChange={handleOnChangeSnackName} label="Type the snack name here!" variant="outlined" />
          {/* Enter snack name:
          <input
            type="text"
            value={snackName}
        ...
        <label>
        <TextField id="snack-url-input" value={snackImage} onChange={handleOnChangeSnackImage} label="Type the url of the snack image here!" variant="outlined" />
          {/* Enter url of snack image:
          <input
            type="text"
            value={snackImage}
        ...
        <label>
          ...
          <Checkbox checked={glutenFree} onChange={handleOnChangeGlutenFree} {...label} />
        </label>
        <label>
          Is your snack vegetarian? 🍅 
          <Checkbox checked={veggie} onChange={handleOnChangeVeggie} {...label} />
        </label>
        <label>
          Is your snack dairy-free? 🥛
          <Checkbox checked={dairyFree} onChange={handleOnChangeDairyFree} {...label} />
        </label>
        <label>
          Is your snack nut-free? 🥜
          <Checkbox checked={nutFree} onChange={handleOnChangeNutFree} {...label} />
        </label>
        <Button variant="contained" onClick={handleSnackSubmit}>Submit</Button>
        </FormControl>
        </Typography>
        </Box>
      </Modal>
